...
import unittest
from unittest.mock import MagicMock, call, patch  # Asegurarse de que 'call' está importado
...
class TestMainWindow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch(
            "customtkinter.windows.widgets.appearance_mode.AppearanceModeTracker.add", MagicMock()
        ), patch("src.gui.main_window.ctk.CTk", MagicMock) as MockCTk, patch(
            "src.gui.main_window.ctk.CTkFrame", return_value=MagicMock()
        ) as MockCTkFrame, patch(
            "src.gui.main_window.ctk.CTkLabel",
            side_effect=[self.mock_file_label, MagicMock(), self.mock_status_label, MagicMock()],
        ) as MockCTkLabel, patch(
            "src.gui.main_window.ctk.CTkOptionMenu", return_value=MagicMock()
        ) as MockCTkOptionMenu, patch(
            "src.gui.main_window.ctk.CTkComboBox", return_value=MagicMock()
        ) as MockCTkComboBox, patch(
            "src.gui.main_window.ctk.CTkButton",
            side_effect=[
                self.mock_select_file_button,
                self.mock_start_transcription_button,
                MagicMock(),
                self.mock_copy_button,
                self.mock_save_txt_button,
                self.mock_save_pdf_button,
            ],
        ) as MockCTkButton, patch(
            "src.gui.main_window.ctk.CTkProgressBar", return_value=self.mock_progress_bar
        ), patch(
            "src.gui.main_window.ctk.CTkTextbox", return_value=self.mock_transcription_textbox
        ):
            ...
            if len(MockCTkButton.call_args_list) > 2:  # Si se creó el reset_button
                reset_button_mock = MockCTkButton.call_args_list[2][0][
                    0
                ]  # El mock real del reset_button
    ...
    def test_select_audio_file(self, mock_askopenfilename):
        ...
        self.mock_transcription_textbox.insert.assert_called_once_with(
            "0.0", "Archivo seleccionado: audio.wav\nPresiona 'Iniciar Transcripción'..."
        )
        ...
        self.mock_status_label.configure.assert_called_once_with(
            text="Archivo seleccionado. Listo para transcribir."
        )
    ...
    def test_start_transcription(self, mock_showwarning, mock_thread):
        ...
        mock_thread.assert_called_once_with(
            ...
            args=(
                self.app.audio_filepath,
                self.app.transcription_queue,
    ...
    def test_handle_fragment_completed_message(self, MockCTkButton):
        ...
        with patch.object(self.app, "copy_specific_fragment") as mock_copy_specific:
            ...
            mock_copy_specific.assert_called_once_with(
                1
            )  # Verificar que se llamó con el fragment_number 1
    ...
    def test_copy_specific_fragment(self, mock_showwarning):
        ...
        self.app.fragment_data = {1: "Texto del fragmento uno.", 2: "Texto del fragmento dos."}
        ...
        self.mock_status_label.configure.assert_called_once_with(
            text="Fragmento 1 copiado al portapapeles."
        )
        mock_showwarning.assert_not_called()
        ...
        self.mock_status_label.configure.assert_called_once_with(
            text="Error: No se encontró el fragmento 3."
        )
        mock_showwarning.assert_called_once_with(
            "Advertencia", "No se encontró el texto para el fragmento 3."
        )

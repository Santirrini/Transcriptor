...
class TestParallelLive(unittest.TestCase):
    ...
    @patch(
        "src.core.transcriber_engine.TranscriberEngine._verify_ffmpeg_available",
        return_value="ffmpeg",
    )
    @patch(
        "src.core.transcriber_engine.TranscriberEngine._should_use_chunked_processing",
        return_value=False,
    )
    @patch("src.core.transcriber_engine.TranscriberEngine._transcribe_single_chunk_sequentially")
    def test_parallel_triggers_chunks(
        self, mock_transcribe_chunk, mock_should_chunk, mock_ffmpeg, mock_duration
    ):
    ...
    @patch("src.core.transcriber_engine.TranscriberEngine._transcribe_single_chunk_sequentially")
    def test_live_transcription_messages(
        self, mock_transcribe_chunk, mock_should_chunk, mock_ffmpeg, mock_duration
    ):

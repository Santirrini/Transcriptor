...
class TestTranscriberEngine(unittest.TestCase):
    ...
    def test_transcribe_audio_threaded(self, mock_perform_transcription, mock_load_model):
        ...
        mock_perform_transcription.return_value = "" # _perform_transcription ahora devuelve una cadena vacía
        ...
        # Verificar que _perform_transcription fue llamada
        mock_perform_transcription.assert_called_once_with(test_audio_path, result_queue, language="en", model_instance=mock_model_instance)
    ...
    def test_transcribe_audio_threaded_error_handling(self, mock_perform_transcription):
        ...
        self.assertIn(error_message, messages[2]["data"]) # Verificar que el mensaje de error está contenido
        ...
        mock_perform_transcription.assert_called_once_with(test_audio_path, result_queue, language="en", model_instance=unittest.mock.ANY)
    ...
    def test_perform_transcription(self):
        ...
        if not os.path.exists(test_audio_path):
             self.skipTest(f"Archivo de audio de prueba no encontrado: {test_audio_path}. Crea este archivo para ejecutar esta prueba.")
        ...
        try:
            ...
            with unittest.mock.patch.object(engine, '_load_model') as mock_load_model:
                ...
                mock_segment = unittest.mock.Mock(text="Test segment text.", duration=1.0, start=0.0, end=1.0)
                ...
                engine._perform_transcription(test_audio_path, test_queue, language="en", model_instance=mock_model_instance)
            ...
            self.assertTrue(any(msg.get("type") == "new_segment" for msg in messages_in_queue), "No se encontró mensaje 'new_segment' en la cola.")
            self.assertTrue(any(msg.get("type") == "transcription_finished" for msg in messages_in_queue), "No se encontró mensaje 'transcription_finished' en la cola.")
            # El texto de los segmentos se verifica en test_perform_transcription_generates_fragments

        except FileNotFoundError:
            self.fail(f"FileNotFoundError: El archivo de audio de prueba no se encontró en {test_audio_path}")
    ...
    def test_save_transcription_pdf(self):
        ...
        try:
            ...
            self.assertTrue(os.path.exists(tmp_filepath))
            self.assertGreater(os.path.getsize(tmp_filepath), 0, "El archivo PDF no debe estar vacío")
    ...
    def test_perform_transcription_generates_fragments(self, mock_os_path_exists, MockWhisperModel):
        ...
        mock_segments = [
            ...
            unittest.mock.Mock(text="Segmento 2.", duration=600.0, start=600.0, end=1200.0), # 10 min
            unittest.mock.Mock(text="Segmento 3.", duration=600.0, start=1200.0, end=1800.0), # 10 min - Completa el primer fragmento
            unittest.mock.Mock(text="Segmento 4.", duration=600.0, start=1800.0, end=2400.0), # 10 min
            unittest.mock.Mock(text="Segmento 5.", duration=600.0, start=2400.0, end=3000.0), # 10 min
            unittest.mock.Mock(text="Segmento 6.", duration=600.0, start=3000.0, end=3600.0), # 10 min - Completa el segundo fragmento
            unittest.mock.Mock(text="Segmento 7.", duration=600.0, start=3600.0, end=4200.0), # 10 min - Último fragmento
        ]
        ...
        engine._perform_transcription(test_audio_path, transcription_queue, language="en", model_instance=mock_model_instance)
        ...
        # Esperamos 7 mensajes "new_segment", 3 mensajes "fragment_completed" y 1 mensaje "transcription_finished"
        self.assertEqual(len(messages), 7 + 3 + 1)

        # Filtrar solo los mensajes de fragmentos completados
        fragment_messages = [msg for msg in messages if msg.get("type") == "fragment_completed"]

        self.assertEqual(len(fragment_messages), 3) # Esperamos 3 fragmentos

        # Verificar el primer fragmento
        self.assertEqual(fragment_messages[0]["fragment_number"], 1)
        self.assertEqual(fragment_messages[0]["fragment_text"].strip(), "Segmento 1. Segmento 2. Segmento 3.")
        self.assertEqual(fragment_messages[0]["start_time_fragment"], 0.0)
        self.assertEqual(fragment_messages[0]["end_time_fragment"], 1800.0)

        # Verificar el segundo fragmento
        self.assertEqual(fragment_messages[1]["fragment_number"], 2)
        self.assertEqual(fragment_messages[1]["fragment_text"].strip(), "Segmento 4. Segmento 5. Segmento 6.")
        self.assertEqual(fragment_messages[1]["start_time_fragment"], 1800.0)
        self.assertEqual(fragment_messages[1]["end_time_fragment"], 3600.0)

        # Verificar el último fragmento
        self.assertEqual(fragment_messages[2]["fragment_number"], 3)
        self.assertEqual(fragment_messages[2]["fragment_text"].strip(), "Segmento 7.")
        self.assertEqual(fragment_messages[2]["start_time_fragment"], 3600.0)
        self.assertEqual(fragment_messages[2]["end_time_fragment"], 4200.0) # El tiempo final del último segmento
